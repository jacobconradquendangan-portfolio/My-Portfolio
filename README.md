# Jacob Conrad Quendangan
### BS Information Systems | Aspiring Solutions Architect & Quality Engineer

<p align="left">
  <img src="https://img.shields.io/badge/CIIT-College%20of%20Arts%20and%20Technology-blue?style=for-the-badge&logo=google-classroom&logoColor=white" />
  <img src="https://img.shields.io/badge/BSIS-3rd%20Year-orange?style=for-the-badge" />
  <img src="https://vercelbadge.vercel.app/api/jacobconradquendangan-portfolio/my-portfolio-six-pi-33?style=for-the-badge" />
</p>

> "Bridging the gap between robust system architecture and data-driven reliability."

---

## Featured Tracks

### [01: Industry Wins & Awards](./01_Industry_Wins_Awards)
* **MNLFlow Transit App:** 1st Place Industry Choice (evaluated by Cebu Pacific).
* **Aegis IDaaS:** 2nd Place Institutional Hackathon (Next.js/Firebase).
* **CESCon Sales Blitz:** #1 Sales Rank in Food Category (Demand Forecasting).

### [02: Systems Architecture & Dev](./02_Systems_Architecture_Dev)
* **Software Testing & Selenium Automation:** Hybrid QE suite featuring 20 automated test cases (Node.js) and full manual documentation.
* **EduTrack Tech Com:** High-performance enrollment prototype with complex prerequisite logic.
* **SAD Hotel Management:** Full-cycle System Analysis & Design with UML/SRS blueprints.
* **Bulls & Cows Engine:** Algorithmic logic and deductive comparison engine in C++.

### [03: Data Analytics & Dashboards](./03_Data_Analytics_Dashboards)
* **Diabetes Prediction:** Python-based ML model, feature engineering, and predictive analysis.
* **BI HR Attrition:** Interactive Power BI & Excel dashboard suite for workforce analytics.
* **EDM Medical SQL:** Relational database schema architecture and Metabase analytics.

---

## Technical Exposure

### **Testing & Quality Engineering**
![Selenium](https://img.shields.io/badge/-Selenium-%2343B02A?style=flat-square&logo=selenium&logoColor=white) 
![Node.js](https://img.shields.io/badge/-Node.js-%23339933?style=flat-square&logo=node.js&logoColor=white) 
![JavaScript](https://img.shields.io/badge/-JavaScript-%23F7DF1E?style=flat-square&logo=javascript&logoColor=black)

### **Data & Analytics**
![Power BI](https://img.shields.io/badge/-Power%20BI-%23F2C811?style=flat-square&logo=power-bi&logoColor=black) 
![Excel](https://img.shields.io/badge/-Excel-%23217346?style=flat-square&logo=microsoft-excel&logoColor=white) 
![Python](https://img.shields.io/badge/-Python-%233776AB?style=flat-square&logo=python&logoColor=white) 
![SQL](https://img.shields.io/badge/-SQL-%23CC2927?style=flat-square&logo=sqlite&logoColor=white)

### **Development & Design**
![React](https://img.shields.io/badge/-React-%2361DAFB?style=flat-square&logo=react&logoColor=black) 
![Next.js](https://img.shields.io/badge/-Next.js-%23000000?style=flat-square&logo=next.js&logoColor=white) 
![Figma](https://img.shields.io/badge/-Figma-%23F24E1E?style=flat-square&logo=figma&logoColor=white)

---
[LinkedIn](https://www.linkedin.com/in/your-profile) | [Live Portfolio](https://portfolio-website-psi-five-95.vercel.app/)